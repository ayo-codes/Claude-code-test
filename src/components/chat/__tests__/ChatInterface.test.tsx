...
test("renders chat interface with message list and input", () => {
  // Provide at least one message so ChatInterface renders MessageList
  // (with 0 messages it renders the empty state instead, which has no message-list testid)
...
test("scrolls when messages change", () => {
  // Start with one message so MessageList (and the ScrollArea) is rendered.
  // With 0 messages, ChatInterface shows the empty state (no ScrollArea).
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
...
test("renders with correct layout classes", () => {
  // Provide messages so MessageList renders inside the ScrollArea —
  // the layout checks below require the scroll area to be present
  (useChat as any).mockReturnValue({
    ...mockUseChat,
    messages: [{ id: "1", role: "user", content: "Hello" }],
  });
